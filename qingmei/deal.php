...
if ($_POST['submit']) {
	...
	if($sex =='male')
		$sex = "男";else $sex = "女";
	$brithday =hg_input_bb( $_POST['user_brithday']);
	...
	if (!empty($name) && !empty($sex) && !empty($brithday) && !empty($phone) && !empty($qq) && !empty($grade) && !empty($college)){
		...
		if(mysql_num_rows($data) == 0){
			$query = "INSERT INTO `info`(`user_id`, `user_name`, `user_sex`, `user_brithday`, `user_phone`, `user_qq`, `user_grade`, `user_college`) VALUES ('','$name','$sex',$brithday,'$phone','$qq','$grade','$college')";
			mysql_query($query);
